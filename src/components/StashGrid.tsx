...
export default function Component() {
                ...
                >
                    <CardHeader className="pb-3">
                        <CardTitle className="text-xl font-bold text-light-1 flex items-center">
                            {stash.isPinned && <Pin className="w-4 h-4 mr-2 text-theme-warning" />}
                            {stash.title}
                        </CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <p className="text-sm text-light-3 mb-4 line-clamp-2 leading-relaxed">{stash.desc}</p>
                        ...
                        {stash.tags.length > 0 && (
                            <div className="flex flex-wrap gap-2 mb-4">
                                {stash.tags.slice(0, 3).map((tag: string, index: number) => (
                                    <span key={index} className="bg-dark-2/60 backdrop-blur-sm border border-dark-3/30 text-light-2 text-xs font-medium px-3 py-1.5 rounded-full flex items-center hover:bg-theme-primary/20 hover:border-theme-primary/40 transition-all duration-300">
                                        <Tag className="w-3 h-3 mr-1" />
                                        {tag}
                                    </span>
                                ))}
                                {stash.tags.length > 3 && (
                                    <span className="bg-theme-primary/20 backdrop-blur-sm border border-theme-primary/30 text-light-2 text-xs font-medium px-3 py-1.5 rounded-full">
                                        +{stash.tags.length - 3}
                                    </span>
                                )}
                            </div>
                        )}
                        <div className="flex justify-between text-sm text-light-3 bg-dark-2/30 backdrop-blur-sm rounded-lg p-3 border border-dark-3/20">
                            <div className="flex items-center">
                                <Code className="w-4 h-4 mr-2 text-theme-success/80" />
                                <span className="font-medium">{stash.stashSections.filter((section: any) => section.type === 'code').length}</span>
                                <span className="ml-1">code snippets</span>
                            </div>
                            <div className="flex items-center">
                                <FileText className="w-4 h-4 mr-2 text-theme-secondary/80" />
                                <span className="font-medium">{stash.stashSections.filter((section: any) => section.type === 'text').length}</span>
    ...
    return (
        <div className="p-6 bg-gradient-to-br from-dark-0 via-dark-1 to-dark-2 min-h-screen">
            <div className="flex flex-col sm:flex-row gap-4 mb-4">
                <div className="flex-grow flex items-center gap-2">
                    <Input
                        ...
                        className="flex-grow bg-dark-1/80 backdrop-blur-sm border-dark-3/50 text-light-1 placeholder-light-4/70 focus:border-theme-primary/50 focus:ring-theme-primary/20 transition-all duration-300"
                    />
                    <Button
                        onClick={handleSearch}
                        disabled={isSearching}
                        className="bg-theme-primary/90 hover:bg-theme-primary text-white border-0 shadow-lg hover:shadow-xl transition-all duration-300 backdrop-blur-sm"
                    >
                        {isSearching ? 'Searching...' : <Search className="w-4 h-4" />}
                    </Button>
                </div>
                <div className="flex items-center justify-between sm:justify-end gap-4">
                    <div className="flex items-center space-x-3 bg-dark-1/50 backdrop-blur-sm rounded-full px-4 py-2 border border-dark-3/30">
                        <span className="text-light-2 text-sm font-medium">Advanced</span>
                        <Switch
                    ...
                    {searchResults.length > 0 && (
                        <Button
                            ...
                            className="bg-dark-2/80 hover:bg-dark-3 text-light-1 border border-dark-3/50 backdrop-blur-sm transition-all duration-300"
                        >
                            View All
                        </Button>
                    )}
                ...
                <div className="pt-4">
                    ...
                    >
                        <AccordionItem value="advanced-options" className="border-none">
                            <AccordionTrigger className="text-light-1 hover:no-underline hover:text-theme-primary transition-colors duration-300 font-semibold">
                                Advanced Search Options
                            </AccordionTrigger>
                            <AccordionContent>
                                <div className="bg-dark-1/60 backdrop-blur-xl p-6 rounded-2xl border border-dark-3/40 shadow-2xl">
                                    <div className="space-y-5">
                                        ...
                                        <RadioGroup
                                            ...
                                            className="space-y-3"
                                        >
                                            <div className="flex items-center space-x-3 transition-all duration-200 hover:bg-dark-2/50 hover:backdrop-blur-sm p-3 rounded-xl cursor-pointer group border border-transparent hover:border-dark-3/30">
                                                <RadioGroupItem
                                                    value="desc"
                                                    id="desc"
                                                ...
                                                <Label
                                                    htmlFor="desc"
                                                    className="text-light-2 group-hover:text-light-1 transition-colors duration-200 cursor-pointer font-medium"
                                                >
                                                    Description
                                                </Label>
                                            </div>
...

            {
                searchResults.length > 0 ? (
                        ...
                        >
                            ...
                            >
                                <div className="flex flex-col items-center justify-center h-full p-8">
                                    <div className="w-16 h-16 bg-theme-primary/20 rounded-2xl flex items-center justify-center mb-4 group-hover:bg-theme-primary/30 transition-all duration-300">
                                        <PlusCircle className="w-8 h-8 text-theme-primary" />
                                    </div>
                                    <p className="text-light-1 text-lg font-semibold">Create New Stash</p>
                                    <p className="text-light-3 text-sm mt-1">Start organizing your code snippets</p>
                                </div>
                            </div>
                    ...
                    <>
                        {pinnedStashes.length > 0 && (
                            <div className="mb-12">
                                <div className="flex justify-between items-center mb-6">
                                    <h2 className="text-3xl font-bold text-light-1">Pinned Stashes</h2>
                                    <Button
                                        onClick={handleRefresh}
                                        disabled={isRefreshing}
                                        className="bg-dark-2/80 hover:bg-dark-3 text-light-1 border border-dark-3/50 backdrop-blur-sm transition-all duration-300 hover:border-theme-primary/40"
                                    >
                                        {isRefreshing ? (
                                            <div className="flex items-center">
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-6">
                                <h2 className="text-3xl font-bold text-light-1">All Stashes</h2>
